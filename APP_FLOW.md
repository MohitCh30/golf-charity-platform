...
## 9. KNOWN ISSUES TO VERIFY IN BROWSER AUDIT

Priority order for OpenCode + browser MCP to check:

1. **Build fix**: remove `~/package-lock.json`, fix `auth/callback` import, redeploy
2. **Auth callback**: after fix, does OAuth redirect actually complete and set session?
3. **Signup Step 3**: does mock payment failure show an error or silently hang?
4. **Dashboard load**: does it redirect correctly if subscription is inactive vs no session?
5. **Score entry**: submit a score, verify DB trigger fires and oldest is dropped on 6th entry
6. **Score column name**: `played_date` in queries — confirm matches actual Supabase column
7. **Admin access**: set role = 'admin' in profiles, verify /admin is accessible
8. **Draw simulation**: runs without publishing, no writes to winners table
9. **Draw publish**: writes winners, cannot be re-published
10. **Charity listing**: loads on /charities without auth
11. **proxy.ts**: confirm it's being picked up as middleware (check Next.js config for the export name)

---

## 10. WHAT IS NOT BUILT (PRD items that are future scope)

- Email notifications (draw results, winner alerts, system updates)
- Mobile hamburger navigation
- Individual charity event pages (upcoming golf days)
- Admin UI for adding/editing charities (currently read-only in admin panel — verify)
- Subscription cancellation/renewal flows with user-facing controls
- Independent donation not tied to subscription
- Multi-country / currency support
- Increased charity % (user can't change it from 10% currently)
